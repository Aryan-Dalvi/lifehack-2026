...
export function MerchantDashboard({ onNavigate }: { onNavigate?: (path: string) => void }) {
  ...
  useEffect(() => {
    ...
    let live = true;
    // Resolve the key to its merchant before requesting insights. The dashboard must never
    // assume the seeded demo tenant now that onboarding is self-serve.
    api<{ merchant_id: string }>("/merchant/me")
      .then((merchant) =>
        api<Insights>(
          `/merchant/${encodeURIComponent(merchant.merchant_id)}/insights?days=${days}`,
        ),
      )
      .then((data) => {
        if (!live) return;
