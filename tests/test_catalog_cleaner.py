...
def test_full_catalog_is_staged_classified_and_only_published_after_approval(
    ...
    product = client.get("/catalog/product/0007").json()
    ...
    assert client.get("/catalog/product/MYSA-CLN-101").status_code == 200
...
def test_replace_plan_is_bound_to_the_live_catalog_revision(client: TestClient) -> None:
    ...
    assert client.get("/catalog/product/B-1").status_code == 200
    assert client.get("/catalog/product/A-1").status_code == 404
