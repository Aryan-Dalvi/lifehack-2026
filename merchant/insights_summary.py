...
def _facts_for(scope: str, insights: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    if scope == "payments":
        payment = insights["payments"]
        summary = (
            f"{kpis['orders']['display']} authorizations settled in the {window}, worth "
            f"{kpis['revenue']['display']}. {payment['note']} The agent never prices a cart or "
            "authorizes a payment, and each order carries a "
        ...
        facts = [
            ...
            f"Payment mode: {payment['mode']}. {payment['note']}",
